...
if [ ${#CONFIGS[@]} -eq 0 ]; then
    ...
    CONFIGS=("w1:1:false" "w50:50:false" "w99:99:false")
fi
...
start_scylla() {
    ...
    local abs_datadir="$(realpath $datadir)"
    local abs_toplevel="$(realpath .)"
    docker run --rm --name scylla-bench \
        ...
        --tinylfu-initial-window-percent "$window_pct" \
        &> "$datadir/scylla.log" &

    log "Waiting for API..."
    for i in $(seq 1 120); do
        if curl -s "$API/cache_service/tinylfu_initial_window_percent" 2>/dev/null | grep -q "[0-9]"; then
    ...
    log "Window: $(curl -s $API/cache_service/tinylfu_initial_window_percent)%"
}

load_data() {
    log "Creating schema and loading data..."
    ~/.local/bin/cqlsh localhost $CQL_PORT -e "CREATE KEYSPACE IF NOT EXISTS $HOT_KEYSPACE WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1};" 2>/dev/null
    ~/.local/bin/cqlsh localhost $CQL_PORT -e "CREATE KEYSPACE IF NOT EXISTS $SCAN_KEYSPACE WITH replication = {'class': 'SimpleStrategy', 'replication_factor': 1};" 2>/dev/null
...
get_stats() {
    ...
    hits=$(echo "$metrics" | grep "^scylla_cache_row_hits" | awk '{printf "%d", $2}')
    misses=$(echo "$metrics" | grep "^scylla_cache_row_misses" | awk '{printf "%d", $2}')
    cache_bytes=$(echo "$metrics" | grep "^scylla_cache_bytes_used" | awk '{printf "%d", $2}')
    cache_partitions=$(echo "$metrics" | grep "^scylla_cache_partitions{" | awk '{printf "%d", $2}')
...
run_test() {
    ...
    # --- Warmup phase ---
    read pre_hits pre_misses pre_bytes pre_parts pre_hot_hits pre_hot_misses pre_scan_hits pre_scan_misses <<< "$(get_stats)"
    log "Pre-warmup: hits=$pre_hits misses=$pre_misses cache=${pre_bytes}B parts=$pre_parts"

    local warmup_start_ms=$(epoch_ms)
    ...
    local warmup_end_ms=$(epoch_ms)

    read pw_hits pw_misses pw_bytes pw_parts pw_hot_hits pw_hot_misses pw_scan_hits pw_scan_misses <<< "$(get_stats)"
    ...
    local attack_start_ms=$(epoch_ms)

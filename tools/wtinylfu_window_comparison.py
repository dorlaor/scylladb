...
workload with zero useful frequency signal. The <strong>hill-climbing algorithm</strong>
can detect this low-hit-rate regime and adapt the window size accordingly.
</div>\n'''
...
exceeds cache size. This is exactly why Caffeine and our implementation include a
<strong>hill-climbing algorithm</strong> that adaptively adjusts the window size
based on observed hit rates — converging toward the optimal ratio for the current workload.
The <code>tinylfu_initial_window_percent</code> and <code>tinylfu_hill_climbing_enabled</code>
config knobs let operators override this for known workload profiles.
</div>\n'''
    html += '</div>\n'

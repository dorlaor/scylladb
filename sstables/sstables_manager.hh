...
private:
    ...
    // Account a reclaimable component that appeared after the SSTable was loaded, so it cannot
    // be counted by re-reading the whole SSTable's size. Used by the pq footer cache, which is
    // populated on the read path.
    void add_reclaimable_memory(size_t bytes);

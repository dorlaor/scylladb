...
void set_cache_service(http_context& ctx, sharded<replica::database>& db, routes& r) {
    ...
    cs::get_tinylfu_initial_window_fraction.set(r, [&db] (std::unique_ptr<http::request> req) {
        double frac = db.local().get_config().tinylfu_initial_window_fraction();
        return make_ready_future<json::json_return_type>(frac);
    });

    cs::set_tinylfu_initial_window_fraction.set(r, [&db] (std::unique_ptr<http::request> req) -> future<json::json_return_type> {
        auto& cfg = const_cast<db::config&>(db.local().get_config());
        try {
            co_await cfg.tinylfu_initial_window_fraction.set_value_on_all_shards(
                req->get_query_param("value"), utils::config_file::config_source::API);
        } catch (...) {
            throw bad_param_exception(fmt::format("{}", std::current_exception()));
        }
...
void unset_cache_service(http_context& ctx, routes& r) {
    ...
    cs::get_tinylfu_initial_window_fraction.unset(r);
    cs::set_tinylfu_initial_window_fraction.unset(r);
}
